...
def _message_limit(max_size: int) -> int:
    ...
    if max_size > MAX_MESSAGE_SIZE:
        raise ValueError(f"max_size cannot exceed the {MAX_MESSAGE_SIZE}-byte hard limit")
    return max_size
...
def _encode_numpy(value: Any) -> Any:
    if isinstance(value, np.ndarray):
        ...
        if value.nbytes > MAX_MESSAGE_SIZE:
            raise MessageTooLargeError(f"numpy array is {value.nbytes} bytes; limit is {MAX_MESSAGE_SIZE}")
        return {
            b"__ndarray__": True,
...
def _decode_numpy(value: dict[Any, Any]) -> Any:
    ...
    allowed = {b"__ndarray__", b"data", b"dtype", b"shape", "__ndarray__", "data", "dtype", "shape"}
    ...
    if expected_size != len(data):
        raise SerializationError(
            f"ndarray byte length mismatch: expected {expected_size}, received {len(data)}"
        )
    try:
...
def packb(payload: Any, *, max_size: int = MAX_MESSAGE_SIZE) -> bytes:
    ...
    try:
        encoded = msgpack.packb(payload, default=_encode_numpy, use_bin_type=True, strict_types=False)
    ...
    if len(encoded) > limit:
        raise MessageTooLargeError(f"encoded message is {len(encoded)} bytes; limit is {limit}")
    return encoded
...
def unpackb(payload: bytes | bytearray | memoryview, *, max_size: int = MAX_MESSAGE_SIZE) -> Any:
    ...
    if len(payload) > limit:
        raise MessageTooLargeError(f"received message is {len(payload)} bytes; limit is {limit}")
    try:
