...
class MetadataValidationError(ValueError):
    """A metadata object does not match the negotiated v3 contract."""
...
def _object(value: Any, path: str, fields: tuple[str, ...]) -> Mapping[str, Any]:
    ...
    if unknown:
        raise MetadataValidationError(f"{path} has unknown fields: {', '.join(unknown)}")
    return value
...
def _wrist_selector(value: Any, path: str) -> dict[str, bool]:
    obj = _object(value, path, ("joint", "eef"))
    return {
        "joint": _boolean(obj["joint"], f"{path}.joint"),
        "eef": _boolean(obj["eef"], f"{path}.eef"),
    }


def _hand_selector(value: Any, path: str) -> dict[str, bool]:
    obj = _object(value, path, ("left", "right"))
    return {
        "left": _boolean(obj["left"], f"{path}.left"),
        "right": _boolean(obj["right"], f"{path}.right"),
    }


def validate_metadata_format(value: Any) -> dict[str, Any]:
    """Validate and return an independent normalized MetadataFormat object."""
    ...
    state_obj = _object(
        obj["state"],
        "metadata_format.state",
        ("history_len", "current", "left_wrist", "right_wrist", "hand_joint"),
    )
    state = {
        "history_len": _integer(
            state_obj["history_len"],
            "metadata_format.state.history_len",
        ),
        "current": _boolean(
            state_obj["current"],
            "metadata_format.state.current",
        ),
        "left_wrist": _wrist_selector(
            state_obj["left_wrist"],
            "metadata_format.state.left_wrist",
        ),
        "right_wrist": _wrist_selector(
            state_obj["right_wrist"],
            "metadata_format.state.right_wrist",
        ),
        "hand_joint": _hand_selector(
            state_obj["hand_joint"],
            "metadata_format.state.hand_joint",
        ),
    }
...
def validate_server_metadata(value: Any) -> dict[str, Any]:
    """Validate the complete response from ``GET /metadata``."""
    ...
    for field in fields:
        if field in {"port", "max_message_size", "metadata_format"}:
            continue
        normalized[field] = _string(
            obj[field],
            f"metadata.{field}",
            nonempty=field not in {"checkpoint_path", "dataset_path", "prompt"},
        )
    for field, expected in expected_strings.items():
        if normalized[field] != expected:
            raise MetadataValidationError(f"metadata.{field} must be {expected}")

    port = _integer(obj["port"], "metadata.port", minimum=1)
    if port > 65535:
        raise MetadataValidationError("metadata.port must be <= 65535")
    max_message_size = _integer(
    ...
    if max_message_size > MAX_MESSAGE_SIZE:
        raise MetadataValidationError(
            "metadata.max_message_size exceeds the 64 MiB client hard limit"
        )
    normalized["port"] = port
    normalized["max_message_size"] = max_message_size
    normalized["metadata_format"] = validate_metadata_format(obj["metadata_format"])
    return normalized


def validate_reset_response(
    value: Any,
    *,
    expected_session_id: str | None = None,
    expected_request_id: int | None = None,
) -> dict[str, Any]:
    """Validate the complete response from ``POST /reset``."""

    obj = _object(
        value,
        "reset_result",
        ("schema", "session_id", "request_id", "reset", "metadata_format"),
    )
    ...
    session_id = _string(
        obj["session_id"],
        "reset_result.session_id",
        nonempty=True,
    )
    request_id = _integer(obj["request_id"], "reset_result.request_id")
    if obj["reset"] is not True:
        raise MetadataValidationError("reset_result.reset must be true")
    if expected_session_id is not None and session_id != expected_session_id:
        raise MetadataValidationError(
            "reset_result.session_id does not match the request"
        )
    if expected_request_id is not None and request_id != expected_request_id:
        raise MetadataValidationError(
            "reset_result.request_id does not match the request"
        )
    return {
        "schema": schema,
